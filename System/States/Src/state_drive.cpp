...
void DriveState::init() {
	...
	// TODO: Get current pose from localization manager
	// Calculate a trajectory to reach the next destination
	trajectory_manager_calculate_trajectory({0, 0, 0}, next_pose, 1.0);
}
...
end_status_t DriveState::run() {
	...
	trajectory_manager_follow_trajectory({0, 0, 0}, &forward_vel_mps_setpoint, &turn_vel_radps_setpoint, &trajectory_complete, &trajectory_off_course);
